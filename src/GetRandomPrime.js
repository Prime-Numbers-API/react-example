...
class GetRandomPrimeResults extends React.Component {
    ...
    render() {
        ...
        for (let key in content) {
            ...
            if (typeof content[key] == "object") {
                ...
                buildHtmlResults += `<th scope="row">${counter}</th>`;
                buildHtmlResults += `<td>${key}</td>`;
                buildHtmlResults += `<td class="${key}">${this.get_random_prime_api_results_details(content[key], key)}</td>`;
                buildHtmlResults += `</tr>`;
            }
            else {
                buildHtmlResults += `<tr class="results-get-random-prime-${key}">`;
...
export default GetRandomPrimeResults
